  ...
  return (
    <div className="space-y-2">
      {data.unknowns.map((u, i) => {
        const isOpen = open === i;
        return (
          <div key={i} className="border border-amber-200 bg-amber-50 rounded-xl overflow-hidden">
            <button
              type="button"
              onClick={() => setOpen(isOpen ? null : i)}
              className="w-full flex items-start justify-between gap-3 px-5 py-3.5 text-left hover:bg-amber-100/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-amber-600 transition-colors"
            >
              <div className="flex items-start gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-amber-900">{u.what_is_unclear}</p>
                  {!isOpen && (
                    <p className="text-xs text-amber-700 mt-0.5 truncate">{u.why_it_matters}</p>
                  )}
                </div>
              </div>
              <svg
                className={`w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5 transition-transform ${isOpen ? "rotate-180" : ""}`}
