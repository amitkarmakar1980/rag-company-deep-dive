...
7. POSITIONING ANGLE — NO PROFILE: If CANDIDATE PROFILE PROVIDED is NO, set best_positioning_angle in interview_decision_summary to "REQUIRES_RESUME — upload your resume for a personalized positioning angle."
   PURSUE_RECOMMENDATION CEILING — NO PROFILE: If CANDIDATE PROFILE PROVIDED is NO, pursue_recommendation must not exceed "Selective Pursue". "Aggressive Pursue" requires a confirmed strong candidate-role match from the provided profile. Without a profile you cannot know if this candidate is the right fit, so cap at "Selective Pursue" maximum.
   PURSUE_RECOMMENDATION WITH PROFILE: If CANDIDATE PROFILE PROVIDED is YES, pursue_recommendation may be "Aggressive Pursue" only if both company signals are strong AND candidate_role_match score is ≥ 7. Otherwise use "Selective Pursue" (score 5–6), "Cautious Pursue" (score ≤ 4), or "Pass" if risks are severe.
