...
function QuestionRow({ item, rank }: { item: QuestionItem; rank?: number }) {
  ...
  return (
    <li className="border border-gray-200 rounded-xl overflow-hidden bg-white">
      <button
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="w-full text-left px-4 py-3.5 flex items-start justify-between gap-3 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-900 transition-colors"
      >
        <div className="flex items-start gap-3 min-w-0">
          ...
          <span className="text-sm text-gray-900 leading-relaxed font-medium">{item.question}</span>
        </div>
        <svg
        ...
        <div className="px-4 pb-4 pt-1 space-y-3 border-t border-gray-100">
          ...
          {item.follow_up && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5">
              <p className="text-xs font-semibold text-gray-500 mb-1">Follow-up if the answer is strong</p>
              <p className="text-xs text-gray-700 italic leading-relaxed">&ldquo;{item.follow_up}&rdquo;</p>
            </div>
          )}
        </div>
      )}
...
export function QuestionsSection({ data }: { data: Data }) {
  ...
  return (
    <div className="space-y-6">
      {mustAsk.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs font-bold uppercase tracking-wider text-gray-900 bg-gray-900 text-white px-2.5 py-1 rounded-full">
              Must Ask
            </span>
            <span className="text-xs text-gray-400">Top {mustAsk.length} — ask these no matter what</span>
          </div>
          <ul className="space-y-2" role="list">
      ...
      {good.length > 0 && (
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Additional Good Questions</p>
          <ul className="space-y-2" role="list">
